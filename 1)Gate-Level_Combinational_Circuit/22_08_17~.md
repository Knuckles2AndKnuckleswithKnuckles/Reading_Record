...
디지털 시스템은 몇개의 작은 서브시스템(subsystem)들로 구성된다. 서브시스템은 하여금 단순한 시스템이나 미리 설계된 부품으로부터 큰 시스템을 만들 수 있게 해준다. 이러한 작업을 수행하기 위해 VHDL은 *부품 예시화(component instantiation)* 라고 알려진 매커니즘을 제공한다. 이러한 타입의 코드를 **구조 설명(structure description)** 이라고 한다.
