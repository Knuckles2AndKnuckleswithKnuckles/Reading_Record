...
# Test Bench
코드가 개발되고 다면, 호스트 컴퓨터에서 코드의 회로 운영에 오류가 없는지, 실제 물리적인 하드웨어에 *합성(synthesized)* 될 수 있는지 *시뮬레이션(simulation)* 될 수 있다. 시뮬레이션은 보통 같은 HDL 프레임워크에서 실행되며, 실제 실험실 벤치를 모방하기 위해 **Test bench** 라는 특별한 프로그램을 만들게 된다. 
...
위 그림에서 **"uut"** 블록은 테스트 대상인 유닛(the unit under test)이고, **"test vector generator"** 블록은 테스트 입력 패턴(testing input patterns)을 생성하며, **"monitor"** 블록은 출력 반응을 검사한다. 
